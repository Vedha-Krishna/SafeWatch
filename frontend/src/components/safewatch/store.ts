...
export const useStore = create<SafeWatchState>((set) => ({
  ...
  loadIncidents: async () => {
    const { data, error } = await supabase
      .from("incidents")
      .select("incident_id, source_platform, raw_text, cleaned_content, status, category, authenticity_score, severity, location_text, latitude, longitude, normalized_time, created_at")
